...
EOD;
    ...
    public function marks_entry() {
        try{
            ...
            $crud->columns('MH_TERM_SEQ','MH_TEST_SEQ','MH_CLASS_SEQ','MH_SUB_SEQ');
...
<td><select class="grade form-control" name="grade[]"><option value="A">A</option><option value="B">B</option><option value="C">C</option><option value="D">D</option></select></td>
...
EOD;
    ...
    public function ajax_update_std_class_table() {
        ...
        $this->db->where('STD_CURRENT_SESSION', $from_class_session);
...
EOD;
    ...
    public function _callback_student_name($value, $row){
        return $this->db
            ->select('ST_FULL_NAME')
            ->get_where('student_details', array('STD_SEQ' => $row->STUDENT))->row()->ST_FULL_NAME;
    }
    ...
    public function fetch_student_on_class_sec(){
        ...
        WHERE `STD_CS_SEQ` = ".$cs." AND `STD_LEFT` = 0 AND `STD_STATUS`= 0 
        AND STD_SEQ NOT IN (SELECT STUDENT FROM progress_report_entry WHERE EXAM_TERM = ".$term.")
        AND (`STD_CURRENT_SESSION` = '".FINANCIAL_YEAR."' OR `STD_CURRENT_SESSION` = '".CURRENT_YEAR."')
