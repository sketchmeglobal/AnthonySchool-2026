    ...
    public function add_monthly_fees($std_id)
    {
        ...
        if (count((array)$row) == 0) { //if student not exists in student table
            ...
            if (count($result) >= '12') { //if student paid all monthly fees for his class
                ...
                // Class_Type 4 = Higher Secondary (11 & 12) → April to March
                // All others (Nursery/Primary/Secondary)   → January to December
                if ($row->Class_Type == 4) {
                    $months_arr = array("April"=>"4","May"=>"5","June"=>"6","July"=>"7","August"=>"8","September"=>"9","October"=>"10","November"=>"11","December"=>"12","January"=>"1","February"=>"2","March"=>"3");
                } else {
                ...
                }, $result); // array of months paid offline

                // Also block months paid online via Easebuzz for the same period
                $fy_parts = explode('-', $year);
                $fy_start = (int)$fy_parts[0];
                $fy_end   = $fy_start + 1;
                if ($row->Class_Type == 4) {
                    // Higher Secondary: April(fy_start) → March(fy_end)
                    $online_paid = $this->db->query("
                        ...
                        WHERE a.student_id = ? AND t.plan_type = 'monthly' AND t.status = 'success'
                          AND ((a.period_year = ? AND a.period_month >= 4)
                               OR (a.period_year = ? AND a.period_month <= 3))
                    ", [$std_id, $fy_start, $fy_end])->result_array();
                } else {
                    // Nursery / Primary / Secondary: January → December (calendar year = fy_start)
                    $online_paid = $this->db->query("
                        SELECT a.period_month
                        FROM tbl_payment_allocation a
                        JOIN tbl_payment_transaction t ON t.id = a.transaction_id
                        WHERE a.student_id = ? AND t.plan_type = 'monthly' AND t.status = 'success'
                          AND a.period_year = ?
                    ", [$std_id, $fy_start])->result_array();
                }
                foreach ($online_paid as $op) {
                    $months_paid_arr[] = (string)$op['period_month'];
                }
