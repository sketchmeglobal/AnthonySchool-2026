    ...
    public function form_add_student()
    {
        ...
        if ($this->input->post('submit') == 'add_student') {
            ...
            if($this->db->insert('student_details', $insertArray)){
                ...
                //create login account for student (default password = registration number)
                $this->db->insert('users', array(
                    'usertype' => 4,
                    'tbl_id'   => $std_id,
                    'username' => $this->input->post('STD_REGNO'),
                    'email'    => $this->input->post('STD_EMAIL'),
                    'pass'     => hash('sha256', str_replace('/', '', $this->input->post('STD_DOB'))),
                    'verified' => 1,
                    'blocked'  => 0,
                ));
