...
class Masters_m extends CI_Model {
    ...
    public function books() {
        try{
            $output = new \stdClass();
            $query = "SELECT BOOK_SEQ,subject.sub_name, Book_Name,Author,Total_Copies FROM `book_master` LEFT JOIN subject ON subject.sub_id = book_master.sub_id";
    ...
    public function grade_setup_edit() {
        try{
            ...
            $crud->required_fields('grd_from','grade');
            $crud->display_as('grd_from','Greater than');
    ...
    public function teachers() {
        try{
            $output = new \stdClass();
            $query = "SELECT TCH_SRLNO,TCH_CODE,TCH_NAME,TCH_PHONE, CONCAT(Class_Name, '-', Sec_Name) AS cto FROM `teacher` LEFT JOIN class_sec_hdr on class_sec_hdr.CS_SEQ = teacher.TCH_CS_SEQ ";
            $output->all_teachers = $this->db->query($query)->result();
            $output->menu_name = 'Teachers';
    ...
    public function teachers_edit() {
        try{
            ...
            $crud->columns('TCH_CODE','TCH_NAME','TCH_PHONE','TCH_CS_SEQ');
            $crud->required_fields('DEPT_CODE','TCH_NAME','TCH_CAT','TCH_CITY','TCH_EMAIL','TCH_PHONE','TCH_SEX','TCH_DOB','TCH_DOJ','TCH_MRTS','TCH_PF_ACCNO','STD_CAT');
            ...
            $crud->display_as('TCH_CODE', 'Teacher Code');
            $crud->display_as('TCH_NAME', 'Teacher Name');
            ...
            $output->section_heading = 'Teachers <small>(Add / Edit / Delete)</small>';
            $output->menu_name = 'Teachers';
            $output->add_button = '';
    ...
    public function _callback_BeforeInsert_teachers($post_array){
        ...
        $data_insert['usertype'] = '3'; //login type teacher
    ...
    public function employees_edit() {
        try{
            ...
            $crud->columns('EMP_CODE','EMP_DEPT','EMP_NAME','EMP_PHONE');
    ...
    public function add_concession_fees($std_id, $val) {
        ...
        if(count((array)$row) == 0) { //if student not exists in student table
            ...
            if(count($result) >= 1) { //if student concession fees already added
                $this->session->set_flashdata('title', 'Zzz!');
                $this->session->set_flashdata('msg', 'Concession fees for that student, already added.');
                return array('type'=>'redirect', 'page'=>'admin/concession_fees');
            } else { //adding concession fees
    ...
    public function student_details() {
        try{
            ...
            $query = "SELECT STD_SEQ, class_sec_hdr.class_sec, STD_ROLLNO, STD_REGNO, STD_SRLNO, ST_FULL_NAME, STD_PH_NO, STD_IMAGE_PATH, STD_SEX FROM student_details LEFT JOIN class_sec_hdr ON student_details.STD_CS_SEQ = class_sec_hdr.CS_SEQ  WHERE student_details.STD_STATUS = 0 ORDER BY STD_CS_SEQ, STD_ROLLNO";
