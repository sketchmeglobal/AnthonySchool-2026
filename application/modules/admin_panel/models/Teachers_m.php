...
public function marks_entry() {
    ...
    public function add_marks() {
        ...
        if($session_id == 1){
            $cls = $this->db->order_by('class_order')->get('class_sec_hdr')->result_array();
            ...
            $query = "SELECT * FROM `class_sec_hdr` WHERE `CS_SEQ` IN (".$cls_teach.") ORDER BY class_order";
...
//     }
    ...
    // if marks header does not exist
    if(empty($marks_hdr_rs)){
        $this->session->set_flashdata('type', 'error');
        $this->session->set_flashdata('title', 'Naa!');
        $this->session->set_flashdata('msg', 'No record found.');
        return array('page'=>'admin/marks_entry');
    ...
    $this->db->select('sd.STD_SEQ, sd.STD_FNAME, sd.STD_MNAME, sd.STD_LNAME, sd.STD_ROLLNO, 
                   md.MD_DTL_SRLNO, md.MD_MARKS, md.MD_GRADE');
    $this->db->from('student_details sd');
    $this->db->join('marks_dtl md', 
                    "md.MD_STD_SEQ = sd.STD_SEQ AND md.MH_HDR_SRLNO = " . $this->db->escape($hdr_id), 
                    'left');
    $this->db->where('sd.STD_CS_SEQ', $marks_hdr_rs->MH_CLASS_SEQ);
    //$this->db->where('sd.STD_LEFT', 0);
    $this->db->where('sd.STD_STATUS', 0);
    $this->db->order_by('CAST(sd.STD_ROLLNO AS UNSIGNED)', 'ASC', FALSE);
    $marks_dtl_rs = $this->db->get()->result_array();
...
EOD;
        ...
        else if($this->input->post('submit') == 'print_progress_report_4'){
            $data = array();
            $class = $this->input->post('class');
            $std_class = $this->input->post('std_class');
            ...
            $data['tab_title'] = 'Progress Report';
            $data['form_type'] = 'progress_report_type_4';
