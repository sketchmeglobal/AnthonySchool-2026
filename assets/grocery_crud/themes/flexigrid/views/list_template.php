...
<div class="flexigrid" style='width: 100%;' data-unique-hash="<?php echo $unique_hash; ?>">
	...
	<div class="sDiv quickSearchBox" id='quickSearchBox'>
        ...
        <div class="sDiv2" style="float: right !important;width: auto;">
        	<?php 

        	// echo "<pre>"; print_r($this->school_list()); die();
        	//echo "<pre>"; print_r($this->custom_uri()); die();
        	if ($this->custom_uri() == 'yearly_fees_report' or $this->custom_uri() == 'monthly_fees_report' or $this->custom_uri() == 'new_admission_fees_report') { ?>
        	<select name="school_list" id="search_field">
				<option value="">All School</option>
				<?php foreach($this->school_list() as $school_list){?>
				<option value="<?php echo $school_list->ct_id?>"><?php echo $school_list->name?></option>
				<?php }?>
			</select>
        	Date: <input type="date" class="qsbsearch_fieldox search_collection_date grocery_date" style="width: 124px;height: 25px;" name="search_collection_date" size="30" id='search_collection_date'> &nbsp;&nbsp;
			...
			<select name="search_field" id="search_field" style="display:none !important;">
				<option value=""><?php echo $this->l('list_search_all');?></option>
				<?php foreach($columns as $column){?>
				<option value="<?php echo $column->field_name?>"><?php echo $column->display_as?>&nbsp;&nbsp;</option>
				<?php }?>
			</select>
            <input style="display: none;" type="button" value="<?php echo $this->l('list_search');?>" class="crud_search btn" id='crud_search'>
		</div>
	</div>
	...
	<div class="pDiv">
		<div class="pDiv2" style="margin:9px;">
			<div class="pGroup">
				<span class="pcontrol">
					<?php //echo $default_per_page; ?>
	...
	<?php echo form_close(); ?>
	<!-- < ?php if ($this->custom_uri() == 'yearly_fees_report' or $this->custom_uri() == 'monthly_fees_report' or $this->custom_uri() == 'new_admission_fees_report') { ?> ?>
	< ?php echo form_open( $ajax_list_url, 'method="post" id="filtering_form" class="filtering_form" autocomplete = "off" data-ajax-list-info-url="'.$ajax_list_info_url.'"'); ?>
